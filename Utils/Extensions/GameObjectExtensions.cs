...
public static class GameObjectExtensions 
{
	...
	/// <summary>
	/// Extension method for GameObject to calculate the combined bounding box of its MeshRenderers and Terrains.
	/// </summary>
	/// <param name="local">Determines whether to calculate bounds in local space (default is false).</param>
	/// <returns>The combined bounding box of all MeshRenderers and Terrains in the GameObject's hierarchy.</returns>
	public static Bounds GetBounds(this GameObject go, bool local = false)
	{
		// Initialize the bounds variable
		Bounds bounds = default;

		// Iterate through all MeshRenderers in the GameObject's hierarchy
		foreach (var renderer in go.GetComponentsInChildren<MeshRenderer>())
		{
			// Get the bounds of the MeshRenderer or MeshFilter's shared mesh
			var rendererBounds = local ? renderer.gameObject.GetComponent<MeshFilter>().sharedMesh.bounds : renderer.bounds;

			// Update the overall bounds based on the calculated bounds
			if (bounds == default)
		...
		// Iterate through all Terrains in the GameObject's hierarchy
		foreach (var terrain in go.GetComponentsInChildren<Terrain>())
		{
			// Calculate bounds based on Terrain size and position
			var terrainBounds = local
				? new Bounds(terrain.terrainData.size / 2, terrain.terrainData.size)
				: new Bounds(terrain.transform.position + terrain.terrainData.size / 2, terrain.terrainData.size);
		...
		// If no bounds were calculated, set the center to the GameObject's position
		if (bounds == default)
